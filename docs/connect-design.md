...
>
> 通过 QUIC 长连接交互式 attach 远程 rmux 会话，实现类似 `ssh + tmux attach` 的体验。
...
#### 6.2.2 交互式终端处理器（新增 `interactive.rs`）
...
pub struct InteractiveSession {
    pub proxy: std::sync::Arc<ProtocolProxy>,  // Arc 共享，避免 Clone
...
/// scrollback 最大 65531 字节。这里设为约 64KB 的上限值。
const SCROLLBACK_BUFFER_SIZE: usize = 65531; // ~64KB (u16 payload limit - 4 bytes header)
...
pub async fn list_sessions(config: &HostConfig, ca_cert_path: &str) -> Result<()> {
    ...
    if let Some(sessions) = response.get("sessions").and_then(|s| s.as_array()) {
        println!("{:<20} {:<10} {:<8} {}", "SESSION", "PANES", "ATTACHED", "CREATED");
        println!("{}", "-".repeat(60));
        for session in sessions {
            println!(
                "{:<20} {:<10} {:<8} {}",
                session["name"].as_str().unwrap_or("-"),
                session["panes"].as_u64().unwrap_or(0),
                if session["attached"].as_bool().unwrap_or(false) { "yes" } else { "no" },
...
**选择 ~64KB（65531 字节）**，参考 Quil 的 500 行 + sshx 的 1MB。

65531 字节受 TLV 帧 u16 负载长度限制（u16::MAX = 65535，减去 scrollback_len 的 u32 头部 4 字节）。大约能存储最近 1000-2000 行终端输出，足够覆盖大多数排查场景。后续可实现为 bridge 可配置项（`bridge.yaml` 中的 `scrollback_buffer_size`）。

> **Phase 2 TODO**：将 `SCROLLBACK_BUFFER_SIZE` 从硬编码常量改为 bridge 配置项。
...
| 优雅断开 | Ctrl+D 或 `exit` 时发送 Detach 消息 | 断开后 session 继续存活 |
...
SESSION              PANES      ATTACHED CREATED
agent-ops            3          no       2026-07-07T08:00:00Z
debug                1          yes      2026-07-07T09:30:00Z

# 使用自定义 hosts 文件
$ agent-ops --hosts-file ./my-hosts.yaml connect staging-01
```
