...
让 AI Agent 操作服务器，两个核心需求：**留痕**和**会话保持**。

SSH 连接一断，会话就没了，操作记录也无迹可寻。agent-ops 解决的就是这两个问题：底层通过 RMUX 实现**终端会话持久化**（断连不丢会话），上层通过 SQLite 审计日志**记录每一次操作**——谁、什么时间、执行了什么命令、成功还是失败。整套系统通过 TLS 加密的 MCP 协议暴露给 AI Agent，开箱即用。
